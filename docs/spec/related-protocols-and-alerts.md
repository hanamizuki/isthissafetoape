...
- Unsubscribe: a public edge function endpoint accepting a subscription id plus an HMAC signature (server secret `HMAC_SECRET`). Valid signature deletes the subscription without login and renders a plain confirmation page.
...
- Auth: standard `Authorization: Bearer <anon key>` (gateway) plus `x-ingest-key: <INGEST_API_KEY>` (the real check; random 64-hex generated by us, shared with the scraper).
