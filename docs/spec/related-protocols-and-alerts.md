...
- Unsubscribe: a public edge function endpoint accepting a subscription id plus an HMAC signature (server secret `HMAC_SECRET`). Valid signature deletes the subscription without login and renders a plain confirmation page. Canonical signature (PR 4's email link builder must reproduce it exactly): `sig = lowercase-hex(HMAC-SHA256(key = HMAC_SECRET, message = utf8(decimal-string(id))))`, no expiry — an unsubscribe link stays valid for the life of the email it was sent in.
